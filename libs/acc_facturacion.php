...
@$idarr = $_REQUEST['arrenda'];
@$pini = $_REQUEST['pini'];
@$pfin = $_REQUEST['pfin'];

@$valor = $_REQUEST['valor'];
@$rec_arr = $_REQUEST['rec_arr'];
@$rec_ad = $_REQUEST['rec_ad'];
@$f_inicio = $_REQUEST['f_inicio'];
@$f_fin = $_REQUEST['f_fin'];

switch ($opc) {
	case 'generar':
		$bus = mysql_query("SELECT * FROM facturacion WHERE Ini_fac='$inicial' ");
		if(mysql_num_rows($bus)==0)
		{
			$fini = explode("-",$inicial);
			$finil = $fini[2];
			if($finil=='16')
				$finil='15';

			$sel  = mysql_query("SELECT * FROM arrendatarios WHERE Fecin_arr like  '%$finil' ");
			while($res =mysql_fetch_object($sel))
			{
				$cod = $res->Cod_inm;
				$sel2 = mysql_query("SELECT * FROM inmuebles WHERE Cod_inm='$cod' ");
				$res2 = mysql_fetch_object($sel2);
				$crea = mysql_query("INSERT INTO facturacion
				      VALUES('','$res->Id_arr','$cod','$res2->Barr_inm','$res2->Dir_inm','$res2->Val_inm','$res->Recrg_arr','$res->Mora_arr','$inicial','$final')  ");
			}
		}
			...
			$fini = explode("-",$inicial);
			$nompdf = $fini[1].'-'. $fini[0];

			$bs = mysql_query("SELECT * from facturacion WHERE Ini_fac='$inicial' ");
			  while($resp = mysql_fetch_array($bs))
			  {
				    ...
				    $ford = calculaFecha("days",4,$inicial);
				    $fextr = calculaFecha("days",12,$inicial);
	...
	// 	if($pfin=='')
	// 		$pfin='%';
	// 	$sel = mysql_query("SELECT * FROM facturacion WHERE Cod_inm like '$codigo' AND Id_arr like '$idarr' AND Ini_fac like '$pini' AND Fin_fac like '$pfin' ORDER BY Id_fac desc");
	...
	// break;
	case 'editar':
		$act = mysql_query("UPDATE facturacion SET Val_inm='$valor',Recrg_arr='$rec_arr',Mora_arr='$rec_ad',Ini_fac='$f_inicio',Fin_fac='$f_fin' WHERE Id_fac='$codigo' ");
		echo'correcto';
	break;
