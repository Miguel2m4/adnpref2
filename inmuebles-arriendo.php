<?php
  ...
  if(@$ban=='')
	...
	where Serv_inm='Arriendo' AND Cod_inm like '$codigo' and Acti_inm='si' and Dep_inm like '$dep' and Ciu_inm like '$ciu'
	and Barr_inm like '$bar' and Tipo_inm like '$tip' and Habit_inm like '$hab' and Ban_inm like '$ban'
	and Cod_inm NOT IN(SELECT Cod_inm FROM arrendatarios ) ") ;
...
$bus = mysql_query("SELECT * from inmuebles
	where Serv_inm='Arriendo' AND Cod_inm like '$codigo' and Acti_inm='si' and Dep_inm like '$dep' and Ciu_inm like '$ciu'
	and Barr_inm like '$bar' and Tipo_inm like '$tip' and Habit_inm like '$hab' and Ban_inm like '$ban'
	and Cod_inm NOT IN(SELECT Cod_inm FROM arrendatarios ) ORDER BY $orden $limit") ;
...
while($row = mysql_fetch_array($bus, MYSQLI_ASSOC)){
  ...
  $list.='<div class="busqueda-inm">
		<div class="imagen-inmuble">
			...
			<h3><span class="inma">Arriendo</span><a href="javascript:void(0)" class="detalle" id="'.$row['Cod_inm'].'">Codigo: '.$row['Cod_inm'].'</a></h3>
...
<title>Inmuebles en Arriendo | INMOBILIARIA ANDAPREF S.A.S</title>
...
<div id="inm-iz">
	<div id="busquedac">
		<h2>Busqueda</h2>
		<div id="buqueda2">
		<form id="filtro1" method="post" action="inmuebles-arriendo">
			<input type="number" placeholder="Codigo" name="codigo" required>
			<input type="submit" value="Buscar">
		</form>
		<form id="filtro2" method="get" action="inmuebles-arriendo">
			<?php
				include("libs/conexion.php");
				echo'<select  name="de">
			...
			<input type="submit" id="reset" value="Reiniciar">
		</div>
	</div>
</div>
<div id="inm-de">
