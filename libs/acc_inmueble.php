...
switch ($opc) {
	case 'crear':
		...
		if(mysql_num_rows($sel)!=0)
		{
			if($nombre_destacada!='')
				...
				$archivador = '';
			$crea = mysql_query("INSERT INTO inmuebles VALUES('$codigo','$idprop','si','$tipo','$mt2','$habitaciones','$banos','$parqueadero','$servicio','$valor','$departamento','$ciudad','$barrio','$direccion','$ux','$uy','$des1','$des2','$archivador') ");
			if($crea==true)
			{
				if($nombre_destacada!='')
				{
	...
	break;
	case 'ligar':
		$crea = mysql_query("INSERT INTO arrendatarios VALUES ('$id_arren','$nombre','$apellido','$email','$telefono','$movil','$codigo','$recargo','$admintr','$moradmintr','$f_inicio','no')");
		if($crea==true)
		{
	...
	case 'editar':
		...
		if(mysql_num_rows($sel)!=0)
		{
			$act = mysql_query("UPDATE inmuebles SET Id_prop='$idprop',Tipo_inm='$tipo',Mt2_inm='$mt2',Habit_inm='$habitaciones',Ban_inm='$banos',Parq_inm='$parqueadero',Serv_inm='$servicio',Val_inm='$valor',Dep_inm='$departamento',Ciu_inm='$ciudad',Barr_inm='$barrio',Dir_inm='$direccion',X_inm='$ux',Y_inm='$uy',Desc1_inm='$des1',Desc2_inm='$des2'
	...
	break;
	case 'desligar':
		$eli = mysql_query("DELETE FROM arrendatarios WHERE Id_arr='$id_arren' ");
		echo 'correcto';
	break;
	default:
		# code...
